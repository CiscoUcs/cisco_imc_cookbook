...
class LsbootLocalStorage < ManagedObject
    ...
    @@consts = LsbootLocalStorageConsts.new()
#    @@naming_props = set([])

    @@mo_meta = {
        "classic" => MoMeta.new("LsbootLocalStorage", "lsbootLocalStorage", "local-storage", VersionMeta::VERSION151F, "InputOutput", 0xf, [], ["admin", "read-only", "user"], ["Get"]),
        "modular" => MoMeta.new("LsbootLocalStorage", "lsbootLocalStorage", "local-storage", VersionMeta::VERSION2013E, "InputOutput", 0xf, [], ["admin", "read-only", "user"], ["Get"])
    }
    ...
    def initialize(parent_mo_or_dn: nil,  **kwargs)
        ...
        super(class_id: "LsbootLocalStorage", parent_mo_or_dn: parent_mo_or_dn, **kwargs)
